...
const useStyles = makeStyles<{ isActive: boolean }>()((theme, { isActive }) => ({
  button: {
    ...
    '&:hover': {
      backgroundColor: isActive ? theme.colors.primaryAlpha10 : theme.colors.accent,
...
const LayoutItem: FC<LayoutListItemProps> = ({ icon, open, path, title, navigation }) => {
  ...
  return (
    <li>
      {open ? (
        ...
        <Tooltip>
          <TooltipTrigger
            render={<button className={classes.button} aria-label={title} onClick={navigation} />}
          >
            {buttonContent}
          </TooltipTrigger>
